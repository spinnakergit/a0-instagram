#!/bin/bash
# Install the Instagram Integration plugin into an Agent Zero instance.
#
# Usage:
...
PLUGIN_DIR="$A0_ROOT/usr/plugins/instagram"

echo "=== Instagram Integration Plugin Installer ==="
...
SYMLINK="$A0_ROOT/plugins/instagram"
if [ ! -e "$SYMLINK" ]; then
...
if [ "$A0_ROOT" = "/a0" ] && [ -d "/git/agent-zero/usr" ]; then
    GIT_PLUGIN="/git/agent-zero/usr/plugins/instagram"
...
echo ""
echo "Next steps:"
echo "  1. Configure credentials in the Instagram Integration plugin settings (WebUI)"
